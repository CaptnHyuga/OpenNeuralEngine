"""
Cache-Optimized Training - Exploit OS File Caching

Key insight: OS caches safetensor files in RAM after first read!
- First load: 1322ms (disk read)  
- Second+ loads: 249ms (OS RAM cache)

This gives us 5x speedup for free by:
1. Pre-warming the OS cache at startup
2. Using pinned memory for fast GPU transfer
3. Direct safetensors GPU loading where possible
...
class CacheOptConfig:
    ...
    prewarm_cache: bool = True  # Pre-read files to warm OS cache
...
class CacheOptimizedLoader:
    """
    Weight loader optimized for OS cache + pinned memory.
    
    Strategy:
    1. Pre-warm OS file cache by reading all needed weights once
    2. Use pinned memory buffers for fast GPU transfer  
    3. Keep handles open for fast repeated access
    """
    
    def __init__(self, model_path: str, device: torch.device, active_layers: List[int], prewarm: bool = True):
        ...
        # Pre-warm OS cache
        if prewarm:
            self._prewarm_cache(active_layers)
    
    def _prewarm_cache(self, active_layers: List[int]):
        """Read all weights for active layers to warm OS file cache."""
        logger.info(f"Pre-warming OS cache for {len(active_layers)} layers...")
        t0 = time.time()
        ...
        for layer_idx in active_layers:
            ...
            for suffix in suffixes:
                ...
                if fname and fname in self._handles:
                    # Just read to warm cache, don't keep on GPU
                    _ = self._handles[fname].get_tensor(name)
        
        logger.info(f"Cache warmed in {time.time()-t0:.1f}s")
    ...
    def load_fast(self, name: str, dtype=torch.float16) -> Optional[torch.Tensor]:
        """Fast load using pinned memory (OS cache should be warm)."""
        fname = self._index["weight_map"].get(name)
    ...
    def load_layer_fast(self, layer_idx: int, dtype=torch.float16) -> Dict[str, torch.Tensor]:
        """Load all weights for a layer using fast pinned transfer."""
        prefix = f"model.layers.{layer_idx}."
        weights = {}
        ...
        for suffix in suffixes:
            w = self.load_fast(prefix + suffix, dtype)
            if w is not None:
...
class CacheOptimizedTrainer:
    """
    Trainer with OS cache pre-warming and pinned memory.
    
    Optimizations:
    1. Pre-warm OS file cache at startup
    2. Pinned memory for fast CPU->GPU transfer
    3. Sparse layers (8 of 40)
    """
    
    def __init__(self, model_path: str, config: Optional[CacheOptConfig] = None):
        self.config = config or CacheOptConfig()
        ...
        # Loader with cache warming
        self.loader = CacheOptimizedLoader(
            model_path, self.device, self.active_layers, 
            prewarm=self.config.prewarm_cache
        )
        ...
        # Embeddings
        embed_w = self.loader.load_fast("model.embed_tokens.weight")
        torch.cuda.synchronize()
        self.embeddings = nn.Embedding.from_pretrained(embed_w, freeze=True)
        
        self.final_ln = self.loader.load_fast("model.norm.weight")
        torch.cuda.synchronize()
        ...
        logger.info("CacheOptimizedTrainer initialized")
    ...
    def train_step(self, input_ids: torch.Tensor) -> float:
        ...
        for layer_idx in self.active_layers:
            weights = self.loader.load_layer_fast(layer_idx)
            torch.cuda.synchronize()
    ...
    def train(self, texts: List[str], epochs: int = 1) -> Dict:
        logger.info(f"Training on {len(texts)} samples (cache optimized)")
...
if __name__ == "__main__":
    ...
    config = CacheOptConfig(max_seq_length=32, prewarm_cache=True)
    trainer = CacheOptimizedTrainer("models/phi-4", config)
